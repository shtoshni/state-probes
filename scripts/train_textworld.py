...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="use pretrained checkpoints saved in local directories",
)
...
train_dataloader = TWFullDataLoader(
    dataset, args.gamefile, tokenizer, batchsize, device=args.device
)
dev_dataloader = TWFullDataLoader(
    dev_dataset, args.gamefile, tokenizer, eval_batchsize, device=args.device
)
...
for i in range(args.epochs):
    if i - best_loss_epoch > args.patience:
        break
    model.train()
    lang_train_losses = []

    for j, (inputs, lang_tgts, init_state, tgt_state, game_ids, entities) in enumerate(
        train_dataloader
    ):
        optimizer.zero_grad()
        return_dict = model(
