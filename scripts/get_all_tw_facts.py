...
def main(
    ...
    if state_model_arch == "bart":
        ...
        tokenizer = BartTokenizerFast.from_pretrained(
            model_fp, local_files_only=local_files_only
        )
    ...
    if state_model_path and state_model_path == "pretrain":
        state_model = model_class.from_pretrained(
            model_fp, local_files_only=local_files_only
        )
    else:
        ...
        if state_model_path:
            state_model.load_state_dict(
                torch.load(state_model_path, map_location=torch.device("cpu"))
            )
    ...
    dev_dataset = TWDataset(
        data_dir, tokenizer, "dev", max_seq_len=float("inf"), max_data_size=float("inf")
    )
    ...
    if "pair" in probe_target:
        ent_set_size = 2
    if "single" in probe_target:
        ent_set_size = 1
    with torch.no_grad():
        probe_outs = gen_all_facts(
            gamefile,
            state_encoder,
            None,
            tokenizer,
    ...
    if "pair" in probe_target:
        ...
        if not os.path.exists(pair_out_file):
            ...
            possible_pairs, type_to_gid_to_ents = gen_possible_pairs(
                gamefile, tqdm(game_ids)
            )
            possible_pairs_serialized = {}
            for gameid in possible_pairs:
                possible_pairs_serialized[gameid] = [
                    EntitySet.serialize(pair) for pair in possible_pairs[gameid]
                ]
            # serialize
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="None, `pretrain`, or filepath to checkpoint",
    )
    parser.add_argument("--state_model_arch", type=str, choices=["t5", "bart"])
    parser.add_argument(
        "--probe_target",
        type=str,
        default="belief_facts_pair",
        choices=["belief_facts_pair", "belief_facts_single"],
    )
    ...
    main(
        ...
        args.gamefile,
        args.out_file,
        args.state_model_path,
        args.state_model_arch,
        args.probe_target,
