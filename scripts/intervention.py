...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="load existing LM checkpoint (if any)",
)
parser.add_argument(
    "--probe_layer", type=int, default=-1, help="which layer of the model to probe"
)
...
def encode_input(inp_sentence=None, tokenized_sentence=None, device="cuda"):
    if tokenized_sentence is None:
        inputs = tokenizer(
            inp_sentence, return_tensors="pt", padding=True, truncation=True
        ).to(device)
    else:
    ...
    if replace_input_ids is not None:
        for btr in beaker_to_replace:
            ...
            if btr == 0:
                ...
                begin_pos = split_pos[btr - 1]
            if btr == 6:
                end_pos = period_pos
            else:
                end_pos = split_pos[btr]

            replace_split_pos = (
                (replace_input_ids == tokenizer.convert_tokens_to_ids(","))
                .nonzero(as_tuple=False)
                .squeeze(-1)
            )
            replace_period_pos = (
            ...
            if btr == 0:
                ...
                replace_begin_pos = replace_split_pos[btr - 1]
            if btr == 6:
                replace_end_pos = period_pos
            else:
                replace_end_pos = replace_split_pos[btr]

            assert (
                replace_input_ids[begin_pos:end_pos][:5]
                == input_ids[begin_pos:end_pos][:5]
            ).all()
            input_ids = torch.cat(
                [
                    input_ids[:begin_pos],
                    replace_input_ids[replace_begin_pos:replace_end_pos],
                    input_ids[end_pos:],
                ],
                dim=0,
            )
            hidden_reps = torch.cat(
                [
                    hidden_reps[:begin_pos, :],
                    replace_hidden_reps[replace_begin_pos:replace_end_pos, :],
                    hidden_reps[end_pos:, :],
                ],
                dim=0,
            )
            hidden_reps_mask = torch.cat(
                [
                    hidden_reps_mask[:begin_pos],
                    replace_hidden_reps_mask[replace_begin_pos:replace_end_pos],
                    hidden_reps_mask[end_pos:],
                ],
                dim=0,
            )
...
def get_log_likelihood_of_samples(base_lm, encoder_outputs, samples):
    if encoder_outputs.size(0) == 1 and samples.size(0) != 1:
        encoder_outputs = torch.cat(samples.size(0) * [encoder_outputs])
    base_lm_token_logits = base_lm(
        input_ids=None, encoder_outputs=(encoder_outputs, None), labels=samples
    )[1]
    base_lm_token_logprobs = base_lm_token_logits.log_softmax(-1)
    target_token_logprobs = base_lm_token_logprobs.gather(
        dim=-1, index=samples.unsqueeze(-1)
    ).squeeze(-1)
...
def make_list_tensor(lst, mask, pad_idx, pad_dim):
    ...
    tensor = [
        F.pad(
            t,
            pad=tuple(
                [0 for _ in range(2 * (len(t.size()) - 1 - pad_dim) + 1)]
                + [max_len - t.size(pad_dim)]
            ),
            value=pad_idx,
        )
        for t in lst
...
def gen_all_actions(action_type):
    ...
    all_beakers = [f"{pos}{color}" for pos in all_positions for color in colors] + list(
        word_to_int.keys()
    )
    all_actions = []
    if action_type == "all" or action_type == "drain":
        all_actions += [
            f"drain {amt} from the {beaker} beaker"
            for amt in range(1, 5)
            for beaker in all_beakers
        ]
    if action_type == "all" or action_type == "pour":
        all_actions += [
            f"pour the {beaker1} beaker to the {beaker2} beaker"
            for beaker1 in all_beakers
            for beaker2 in all_beakers
        ]
...
def create_new_sentence(
    next_utt, priorTxt, priorTxt_rawstate, orig_final_state_raw, create_type
):
    """
    ...
    try:
        ...
        target_beaker_pos = action_args[
            0
        ]  # draining from, pouring from, or mixing this beaker (0-indexed)
    except (KeyError, AssertionError) as e:
        # random nonempty beaker
        for target_beaker_pos in range(7):
            if (
                orig_final_state_raw["objects"][target_beaker_pos] is not None
                and len(orig_final_state_raw["objects"][target_beaker_pos]) > 0
            ):
                break
    assert (
        orig_final_state_raw["objects"][target_beaker_pos] is not None
    )  # since we'll be doing an action on it...
    ...
    new_sentence, new_sentence_rawstate, new_final_state_raw = (
        priorTxt,
        priorTxt_rawstate,
        orig_final_state_raw,
    )
    split_token = ".\n" if ".\n" in priorTxt else ". "
...
def create_orig_sentence(
    new_next_utt,
    new_sentence,
    new_sentence_rawstate,
    new_final_state_raw,
    priorTxt,
    priorTxt_rawstate,
    orig_final_state_raw,
    ...
    if consistencyCheck(new_sentence_rawstate, new_next_utt):
        ...
        action, action_args = parse_utt_with_world(
            new_next_utt[:-1], new_final_state_raw
        )
        orig_target_beaker_pos = action_args[
            0
        ]  # draining from, pouring from, or mixing this beaker (0-indexed)
    else:
        # random nonempty beaker
        for orig_target_beaker_pos in range(7):
            if (
                new_final_state_raw["objects"][orig_target_beaker_pos] is not None
                and len(new_final_state_raw["objects"][orig_target_beaker_pos]) > 0
            ):
                break
    target_beaker_amt = len(orig_final_state_raw["objects"][orig_target_beaker_pos])
    if create_type == "drain_1":
        assert target_beaker_amt == len(
            new_final_state_raw["objects"][orig_target_beaker_pos]
        )
...
dev_dataset, lang_v_dev, state_v_dev = loadData(
    split="dev", kind="alchemy", synthetic=True
)
best_val_loss = 10**10
best_epoch = -1
base_lm.eval()
...
for j, (inputs, lang_tgts, probe_outs, raw_state_targets, init_states) in enumerate(
    convert_to_transformer_batches(
        dev_dataset,
        tokenizer,
        EVAL_BATCHSIZE,
        include_init_state=encode_init_state,
        domain="alchemy",
        device=args.device,
    )
):
    print(f"BATCH {j}/{tot_num_batches}")
    encoder_outputs, input_ids, attention_mask = encode_input(
        tokenized_sentence=inputs, device=args.device
    )
    hidden_reps_mask = attention_mask.clone()
    orig_output = base_lm.generate(
        input_ids=input_ids,
        decoder_start_token_id=base_lm.config.pad_token_id,
        max_length=128,
    ...
    for idx, final_state in enumerate(tqdm(raw_state_targets["full_state"])):
        ...
        orig_final_state_raw = parse_world(
            " ".join(i[2:] for i in orig_final_state_raw.split(" "))
        )
        # skip if <2 nonempty (>=2 to create the 2 sentences)
        num_nonempty = len(orig_final_state_raw["objects"]) - orig_final_state_raw[
            "objects"
        ].count(None)
        if num_nonempty < 2:
        ...
        assert tokenizer.decode(input_ids[idx], skip_special_tokens=True).replace(
            "  ", " "
        ).replace("\n", " ") == priorTxt.replace("  ", " ").replace("\n", " ")
        if "." in priorTxt:
            priorTxt_rawstate = (
                init_states[idx] + ". " + priorTxt[priorTxt.index(".") + 2 :]
            )
        else:
            priorTxt_rawstate = init_states[idx] + ". " + priorTxt
        result["priorTxt"] = priorTxt_rawstate
        result["orig_utt"] = orig_utt
        orig_hidden_reps, orig_input_ids, orig_attn_mask = (
            encoder_outputs.last_hidden_state[idx],
            input_ids[idx],
            attention_mask[idx],
        )
        # create new sentence
        ...
        new_encoder_outputs, new_input_ids, new_attention_mask = encode_input(
            new_sentence
        )
        new_utt = tokenizer.decode(
            base_lm.generate(
        ...
        if update_orig:
            ...
            orig_encoder_outputs, orig_input_ids, orig_attn_mask = encode_input(
                priorTxt
            )
            result["priorTxt"] = priorTxt_rawstate
            result["orig_utt"] = tokenizer.decode(
                base_lm.generate(
                    encoder_outputs=orig_encoder_outputs,
                    encoder_attention_mask=orig_attn_mask,
                    decoder_start_token_id=base_lm.config.pad_token_id,
                    max_length=128,
                    no_repeat_ngram_size=0,
                )[0],
            ...
            orig_hidden_reps, orig_input_ids, orig_attn_mask = (
                orig_encoder_outputs.last_hidden_state[0],
                orig_input_ids[0],
                orig_attn_mask[0],
            )
        ...
        hidden_reps_mix, attention_mask_mix = replace_tokens(
            ...
            beaker_to_replace=target_beaker_pos,
        )
        # get metric
        mix_encoder_outputs = ModelOutput(
            last_hidden_state=hidden_reps_mix.unsqueeze(0)
        )
        orig_new_mix_utt = tokenizer.decode(
            base_lm.generate(
                encoder_outputs=mix_encoder_outputs,
                encoder_attention_mask=attention_mask_mix,
                decoder_start_token_id=base_lm.config.pad_token_id,
                max_length=128,
                no_repeat_ngram_size=0,
            )[0],
...
for i, result in enumerate(all_results):
    ...
    num_new_orig_mix_consistent += int(
        consistencyCheck(new_sentence_rawstate, orig_new_mix_utt)
    )
    ...
    if consistencyCheck(new_sentence_rawstate, new_utt):
        num_mix_consistent_if_new_consistent += int(
            consistencyCheck(new_sentence_rawstate, orig_new_mix_utt)
        )
    ...
    num_mix_consistent_orig_world += int(
        consistencyCheck(priorTxt_rawstate, orig_new_mix_utt)
    )

    num_consistent_w_orig_and_new[0] += int(
        consistencyCheck(priorTxt_rawstate, orig_utt)
        and consistencyCheck(new_sentence_rawstate, orig_utt)
    )
    num_consistent_w_orig_and_new[1] += int(
        consistencyCheck(priorTxt_rawstate, new_utt)
        and consistencyCheck(new_sentence_rawstate, new_utt)
    )
    num_consistent_w_orig_and_new[2] += int(
        consistencyCheck(priorTxt_rawstate, orig_new_mix_utt)
        and consistencyCheck(new_sentence_rawstate, orig_new_mix_utt)
    )

print("statement/world")
print(
    f"orig/orig: {num_orig_consistent} / {num_total} = {num_orig_consistent / num_total}"
)
print(
    f"mix/orig: {num_mix_consistent_orig_world} / {num_total} = {num_mix_consistent_orig_world / num_total}"
)
print(
    f"new/orig: {num_new_consistent_orig} / {num_total} = {num_new_consistent_orig / num_total}"
)
print(
    f"orig/new: {num_orig_consistent_new} / {num_total} = {num_orig_consistent_new / num_total}"
)
print(
    f"mix/new: {num_new_orig_mix_consistent} / {num_total} = {num_new_orig_mix_consistent / num_total}"
)
print(f"new/new: {num_new_consistent} / {num_total} = {num_new_consistent / num_total}")
print(
    f"new=mix: {num_new_and_mix_same} / {num_total} = {num_new_and_mix_same / num_total}"
)
print(
    f"(mix if new)/new: {num_mix_consistent_if_new_consistent} / {num_new_consistent} = {num_mix_consistent_if_new_consistent / num_new_consistent}"
)
