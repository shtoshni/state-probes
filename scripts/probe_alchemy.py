...
from data.alchemy.utils import (
    int_to_word,
    gen_all_beaker_states,
    get_matching_state_labels,
...
from probe_models import (
    ProbeLinearModel,
    ProbeConditionalGenerationModel,
    ProbeLanguageEncoder,
    encode_target_states,
    get_probe_model,
    get_state_encoder,
    get_lang_model,
)
...
parser.add_argument(
    "--tgt_agg_method",
    type=str,
    choices=["sum", "avg", "first", "last", "lin_attn", "ffn_attn", "self_attn"],
    default="avg",
...
parser.add_argument(
    "--probe_type",
    type=str,
    choices=["linear", "mlp", "lstm", "decoder"],
    default="linear",
)
parser.add_argument(
...
parser.add_argument(
    ...
    + [
        f"{target}.{text_type}"
        for target, text_type in itertools.product(
            [
                "state",
                "init_state",
                "single_beaker_init",
    ...
    default="single_beaker_final.NL",
    help="what to probe for",
)
parser.add_argument(
    "--probe_agg_method",
    type=str,
...
parser.add_argument(
    "--localizer_type",
    type=str,
    default="single_beaker_init_full",
    choices=["all", "init_state"]
    + single_beaker_opts
    + [f"{opt}.offset{i}" for opt in single_beaker_opts for i in range(7)]
    + [
        f"single_beaker_{occurrence}{token_offset}{offset}"
        for occurrence in ["init", "init_full"]
        for offset in [""] + [f".offset{i}" for i in range(7)]
        for token_offset in [""] + [f".R{j}" for j in range(-7, 8)]
    ],
...
if probe_target != "text":
    similarity_fn = lambda x, y, z: get_state_similarity(x, y, z, probe_target)
else:
    similarity_fn = lambda x, y, z: Levenshtein.ratio(x, y)
...
# load (language) model
model, encoder, tokenizer = get_lang_model(
    arch, lm_save_path, pretrained, device=args.device
)
...
if encode_tgt_state:
    state_model = get_state_encoder(
        encode_tgt_state.split(".")[-1],
        encoder,
        config=model.config,
        pretrained=pretrained,
        freeze_params=(encode_tgt_state.split(".")[1] != "raw"),
        device=args.device,
    )
...
if lm_save_path:
    output_dir = (
        f"{f'encoded_{encode_tgt_state}' if encode_tgt_state else ''}{probe_type}"
    ...
    output_dir = (
        f"{'nopt_' if not pretrained else ''}noft_{f'encoded_{encode_tgt_state}' if encode_tgt_state else ''}{probe_type}"
        + f"_{localizer_type}{'.control_inp' if args.control_input else ''}_"
        + f"{probe_agg_method}{probe_attn_dim if probe_agg_method and probe_agg_method.endswith('_attn') else ''}{probe_max_tokens if probe_max_tokens else ''}{tgt_agg_method if encode_tgt_state else ''}"
...
if not os.path.exists(
    os.path.join("probe_models_alchemy", f"{'/'.join(output_dir.split('/')[:-1])}")
):
    os.makedirs(
        os.path.join("probe_models_alchemy", f"{'/'.join(output_dir.split('/')[:-1])}"),
        exist_ok=True,
...
localizer = SconeLocalizer(
    getattr(probe_model, "agg_layer", None),
    args.probe_agg_method,
    args.probe_attn_dim,
    args.localizer_type,
...
state_localizer = SconeLocalizer(
    getattr(probe_model, "target_agg_layer", None),
    args.tgt_agg_method,
    args.probe_attn_dim,
    "all",
    args.probe_max_tokens,
...
full_joint_model = ProbeLinearModel(
    args.arch,
    getattr(probe_model, "config", getattr(model, "config", None)),
    model,
    state_model,
    probe_model,
    args.probe_layer,
...
# load data
print("Loading data")
dataset, lang_v, state_v = loadData(
    split="train", kind="alchemy", synthetic=(not args.nonsynthetic)
)
dev_dataset, lang_v_dev, state_v_dev = loadData(
    split="dev", kind="alchemy", synthetic=(not args.nonsynthetic)
)
best_val_loss = 10**10
best_epoch = -1
# all_train_states = [" ".join(state) for _, _, state in [x for d in dataset for x in d.all_pairs()]]
all_dev_states = [
    " ".join(state) for _, _, state in [x for d in dev_dataset for x in d.all_pairs()]
]
...
if encode_tgt_state:
    ...
    with torch.no_grad():
        ...
        all_beaker_states, beaker_state_to_idx = gen_all_beaker_states(
            "alchemy", args, encoding=encoding, tokenizer=tokenizer, device=args.device
        )
        # don't use agg here if you need to learn custom weights (i.e. using attn)
        # (otherwise have to re-encode each training step...)
        probe_agg_method = (
            args.probe_agg_method
            if args.probe_agg_method == "sum" or args.probe_agg_method == "avg"
            else None
        )
        if encoding == "NL":
            (
                ...
                probe_model,
                args,
                all_state_targets=all_beaker_states,
            )
        else:
            # (numtotal,1,encodedim)
            all_state_input_ids = torch.stack(all_beaker_states, dim=0).unsqueeze(1)
            all_state_attn_mask = torch.ones(all_state_input_ids.size(0), 1).to(
                args.device
            )
            all_beaker_states = all_state_input_ids.tolist()
        beaker_idx_to_state = {
            beaker_state_to_idx[state]: state for state in beaker_state_to_idx
        }
...
with torch.no_grad():
    ...
    for j, (inputs, lang_tgts, probe_outs, raw_state_targets, init_states) in enumerate(
        tqdm(
            convert_to_transformer_batches(
                ...
                append_last_state_to_context=args.append_last_state_to_context,
                domain="alchemy",
                state_targets_type=probe_target,
                device=args.device,
        ...
        #     probe_outs['all_states_attn_mask'] = all_state_attn_mask
        probe_outs["labels"] = get_matching_state_labels(
            all_beaker_states,
            beaker_state_to_idx,
            probe_outs,
            tokenizer,
        ...
        model_outs = full_joint_model(
            inputs["input_ids"],
            inputs["attention_mask"],
            offset_mapping=inputs["offset_mapping"],
            probe_outs=probe_outs,
            localizer_key=inputs["state_key"],
        )
        ...
        if not encode_tgt_state:
            ...
            generated_ws = full_joint_model.generate(
                ...
                encoder_outputs=model_outs,
                max_length=128,
                decoder_start_token_id=probe_model.config.pad_token_id,
        ...
        for k in range(len(inputs["input_ids"])):
            ...
            if not encode_tgt_state:
                ...
                genState = beaker_idx_to_state[
                    all_beaker_states["input_ids"]
                    .tolist()
                    .index(generated_ws[k].tolist())
                ]
            else:
                genState = beaker_idx_to_state[
                    all_beaker_states.index(generated_ws[k].tolist())
                ]
            gtState = tokenizer.decode(
                probe_outs["input_ids"][k], skip_special_tokens=True
            )
            ...
            all_examples_output["prior"].append(
                tokenizer.decode(inputs["input_ids"][k], skip_special_tokens=True)
            )
    ...
    decoder_metrics = (
        f", fraction {probe_target} match: {n_ratio_state_match/n_val}, "
        + (
            f"percent init state match: {n_init_state_match/n_val}, "
            if probe_target == "state"
            else ""
        )
        + f"percent exact match: {n_exact_state_match/n_val}"
    )
    print(f"avg {probe_target} val loss {avg_probe_val_loss}{decoder_metrics}")
# save states
...
for i in range(args.epochs):
    if (i - best_epoch > args.patience) and (i - best_epoch > args.patience):
        break
    model.train()
    probe_model.train()
    if encode_tgt_state:
    ...
    for j, (inputs, lang_tgts, probe_outs, raw_state_targets, init_states) in enumerate(
        ...
        if j % 100 == 0:
            print(
                f"epoch {i}, batch {j}, {probe_target} score {probe_loss.item()}",
                flush=True,
            )
    print(
        f"epoch {i}, average {probe_target} loss {sum(probe_train_losses).item()/len(probe_train_losses) if len(probe_train_losses) > 0 else 0}"
    )
    ...
    with torch.no_grad():
        ...
        for j, (
            ...
            # else:
            similarity_scores = model_outs["similarity"]
            n_exact_state_match += int(
                (similarity_scores.argmax(-1) == probe_outs["labels"]).sum()
            )
        print("n_val", n_val)
        ...
        if not encode_tgt_state:
            ...
            decoder_metrics = f", accuracy: {n_exact_state_match/n_val}"
        print(
            f"epoch {i}, avg {probe_target} val loss {avg_probe_val_loss}{decoder_metrics}"
        )
        if best_n_exact_state_match < n_exact_state_match or (
            best_n_exact_state_match == n_exact_state_match
            and (
                (not encode_tgt_state and best_n_similarity < n_ratio_state_match)
                or (encode_tgt_state and probe_val_loss < best_val_loss)
            )
        ):
            print(
                f"NEW BEST MODEL: {best_n_exact_state_match/n_val} -> {n_exact_state_match/n_val}"
            )
            model.epoch = i
            best_n_exact_state_match = n_exact_state_match
            best_n_similarity = n_ratio_state_match
