...
from data.textworld.parse_tw import (
    translate_inv_items_to_str,
    translate_inv_str_to_items,
)
from data.textworld.utils import (
    EntitySet,
    control_mention_to_tgt_simple,
...
parser.add_argument(
    ...
    default=False,
    action="store_true",
    help="control inputs to tokenized entity pair",
)
...
parser.add_argument(
    ...
    type=str,
    choices=["em", "loss"],
    help="which metric to use on dev set",
    default="em",
)
...
parser.add_argument(
    "--probe_type",
    type=str,
    choices=["3linear_classify", "linear_classify", "linear_retrieve", "decoder"],
    default="decoder",
)
parser.add_argument(
    "--encode_tgt_state",
...
parser.add_argument(
    "--probe_target",
    type=str,
    default="final.belief_facts",
    choices=list(
        chain(
            *[
                [
                    f"{init_final}.full_facts",
                    f"{init_final}.full_belief_facts",
                    f"{init_final}.belief_facts",
                    f"{init_final}.belief_facts_single",
                    f"{init_final}.belief_facts_pair",
                    f"{init_final}.full_belief_facts_single",
                    f"{init_final}.full_belief_facts_pair",
                    f"{init_final}.belief_facts_single.control",
                    f"{init_final}.full_belief_facts_single.control",
                    f"{init_final}.belief_facts_single.control_with_rooms",
                    f"{init_final}.full_belief_facts_single.control_with_rooms",
                    f"{init_final}.belief_facts_pair.control",
                    f"{init_final}.full_belief_facts_pair.control",
                    f"{init_final}.belief_facts_pair.control_with_rooms",
                    f"{init_final}.full_belief_facts_pair.control_with_rooms",
                ]
...
parser.add_argument(
    ...
    choices=["all"]
    + [
        f"belief_facts_{sides}_{agg}"
        for sides in ["single", "pair"]
        for agg in ["all", "first", "last"]
    ],
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="Filepath to precomputed state vectors",
)
...
metric = args.metric
control_mention_to_tgt, control_mention_to_tgt_with_rooms = (
    control_mention_to_tgt_simple,
    control_mention_to_tgt_with_rooms_simple,
)
    ...
    with torch.no_grad():
        ...
        for j, (
            ...
            model_outs = model(
                ...
                probe_outs=tgt_state,
                localizer_key=entity_sets["mentions"],
            )
            ...
            if not encode_tgt_state:
                ...
                generated_state = model.generate(
                    encoder_outputs=model_outs["decoder_inputs"],
                    max_length=128,
                    decoder_start_token_id=model.probe_base_model.config.pad_token_id,
                    do_sample=False,
                ...
                if retrieve:
                    ...
                    save_dict.append(
                        {"score": similarity_scores, "labels": tgt_state["labels"]}
                    )
                    ...
                    if cms is None:
                        ...
                        cms = [
                            [cm[i][j] + cms[i][j] for j in range(len(cms[i]))]
                    ...
                    for i, item in enumerate(pred_matches):
                        ...
                        tgt_state["all_states_input_ids"] = tgt_state[
                            "all_states_input_ids"
                        ].view(bs, -1, tgt_state["all_states_input_ids"].size(-1))
                        idx_to_state = [
                            tokenizer.decode(neg_inp_ids, skip_special_tokens=True)
                            for neg_inp_ids in tgt_state["all_states_input_ids"][i]
                        ]
                        generated_state.append(
                            {
                                "true": [
                                    idx_to_state[idx]
                                    for idx in (item == 1).nonzero(as_tuple=False)[:, 0]
                                ],
                                "false": [
                                    idx_to_state[idx]
                                    for idx in (item == 2).nonzero(as_tuple=False)[:, 0]
                                ],
                            }
                        )
                        gold_state.append(
                            {
                                "true": [
                                    idx_to_state[idx]
                                    for idx in (tgt_state["labels"][i] == 1).nonzero(
                                        as_tuple=False
                                    )[:, 0]
                                ],
                                "false": [
                                    idx_to_state[idx]
                                    for idx in (tgt_state["labels"][i] == 2).nonzero(
                                        as_tuple=False
                                    )[:, 0]
                                ],
                            }
            ...
            for i, gi in enumerate(generated_state):
                if len(gold_state) > i:
                    gt_state = gold_state[i]
                gen_state = generated_state[i]
                if not encode_tgt_state:  # otherwise already added
                    gt_state = tokenizer.decode(
                        tgt_state["input_ids"][i], skip_special_tokens=True
                    )
                ...
                if output_json_fn:
                    ...
                    if entity_sets:
                        saved_preds[-1]["entity"] = EntitySet.serialize(
                            entity_sets["entities"][i]
                        )
...
output_dir = os.path.join(
    save_dir,
    f"{'enctgt_' if encode_tgt_state else ''}{probe_type}"
...
model, encoder, tokenizer = get_lang_model(
    ...
    n_layers=args.override_num_layers,
    device=args.device,
)
...
optimizer = AdamW(
    [p for p in full_joint_model.parameters() if p.requires_grad], lr=args.lr
)
print("Loaded model")
...
if args.eval_only:
    ...
    output_json_fn = os.path.join(
        output_json_dir, f"{os.path.split(probe_save_path)[-1].replace('.p', '.jsonl')}"
    )
...
if probe_target[1].endswith("_pair"):
    ...
    possible_pairs = load_possible_pairs(
        pair_out_file=os.path.join(args.data, "entity_pairs.json")
    )
    assert possible_pairs is not None
if probe_target[1].endswith("_single"):
    ent_set_size = 1
...
dev_dataset = TWEntitySetDataset(
    ...
    ent_set_size=ent_set_size,
    control=control,
    gamefile=args.gamefile,
    state_key=state_key,
    tgt_state_key=tgt_state_key,
    max_data_size=max_data_size[0],
    inform7_game=inform7_game,
    possible_pairs=possible_pairs,
    precomputed_negs=precomputed_negs,
)
...
train_dataloader = TWEntitySetDataLoader(
    dataset, tokenizer, batchsize, control_input, device=args.device
)
dev_dataloader = TWEntitySetDataLoader(
    dev_dataset, tokenizer, eval_batchsize, control_input, device=args.device
)
...
n_val, avg_val_loss, avg_em, avg_overlap = eval_model(
    args,
    full_joint_model,
    dev_dataloader,
    tokenizer,
...
print(
    f"INIT, avg val loss: {avg_val_loss}, avg em: {avg_em}, avg overlap: {avg_overlap}"
)
...
for i in range(args.epochs):
    ...
    for j, (
        ...
        if j % 1000 == 0 and j != 0:
            ...
            print(
                f"epoch {i} update {j}, avg val loss: {avg_val_loss}, avg em: {avg_em}, avg overlap: {avg_overlap}"
            )
            if (metric == "em" and avg_em >= best_em) or (
                metric == "loss" and avg_val_loss <= best_val_loss
            ):
    ...
    print(
        f"epoch {i}, avg val loss: {avg_val_loss}, avg em: {avg_em}, avg overlap: {avg_overlap}"
    )
    if (metric == "em" and avg_em >= best_em) or (
        metric == "loss" and avg_val_loss <= best_val_loss
    ):
