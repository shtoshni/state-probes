...
def get_prf1(
    ...
    # T precision
    num_true_all_fact_overlap = len(
        set(relevant_full_true_facts).intersection(set(pred_true_facts))
    )
    num_true_pred = len(set(pred_true_facts))
    true_precision = (
        num_true_all_fact_overlap / num_true_pred if num_true_pred > 0 else 1.0
    )
    num_true_gt_fact_overlap = len(
        set(gt_true_facts).intersection(set(pred_true_facts))
    )
    num_true_gt = len(set(gt_true_facts))
    true_recall = num_true_gt_fact_overlap / num_true_gt if num_true_gt > 0 else 1.0
    true_f1 = (
        2 * true_precision * true_recall / (true_precision + true_recall)
    ...
    num_false_all_fact_overlap = len(
        set(pred_false_facts) - set(relevant_full_true_facts)
    )
    num_false_pred = len(set(pred_false_facts))
    false_precision = (
        num_false_all_fact_overlap / num_false_pred if num_false_pred > 0 else 1.0
    )
    num_false_gt_fact_overlap = len(
        set(gt_false_facts).intersection(set(pred_false_facts))
    )
    num_false_gt = len(set(gt_false_facts))
    false_recall = num_false_gt_fact_overlap / num_false_gt if num_false_gt > 0 else 1.0
    false_f1 = (
        2 * false_precision * false_recall / (false_precision + false_recall)
        if false_precision + false_recall > 0
        else 0
    )
    return [true_precision, true_recall, true_f1], [
        false_precision,
        false_recall,
        false_f1,
    ]
...
def split_relational_property_facts(
    all_facts, game_id, game_state=None, inform7_game=None, cached_templates=None
):
    if len(all_facts) == 0:
    ...
    for f, fact in enumerate(all_facts):
        ...
        if (
            "is closed" in fact
            or "is open" in fact
            or "is locked" in fact
            or "is nowhere" in fact
            or "is edible" in fact
        ):
        ...
        if (
            ...
            or "is on " in fact
            or "is at " in fact
            or "The player carries " in fact
            or "The matching key" in fact
            or "is mapped " in fact
        ):
            rel_facts.append(fact)
            is_relation = True
        assert not (is_property and is_relation)
        if not is_property and not is_relation:
            parsed_fact, cached_templates = parse_nl_to_facts(
                [fact],
                game_state,
                game_id,
            ...
            if len(parsed_fact) == 0:
                ...
                if len(parsed_fact["arguments"]) == 2:
                    rel_facts.append(fact)
                elif len(parsed_fact["arguments"]) == 1:
                    property_facts.append(fact)
                else:
                    non_wellformed_facts.append(fact)
...
def get_rel_prop_prf1(
    ...
    (
        rel_full_gt_state,
        prop_full_gt_state,
        non_wellformed_gt_state,
        cached_templates,
    ) = split_relational_property_facts(
        full_gt_state,
        game_id,
    ...
    rel_queried_true_metrics, rel_queried_false_metrics = get_prf1(
        rel_full_gt_state,
        rel_states["queried_gen_state"]["true"],
        rel_states["queried_gen_state"]["false"],
        rel_states["queried_gt_state"]["true"],
        rel_states["queried_gt_state"]["false"],
    )
    prop_queried_true_metrics, prop_queried_false_metrics = get_prf1(
        prop_full_gt_state,
        prop_states["queried_gen_state"]["true"],
        prop_states["queried_gen_state"]["false"],
        prop_states["queried_gt_state"]["true"],
        prop_states["queried_gt_state"]["false"],
    )
    return (
        rel_queried_true_metrics,
        rel_queried_false_metrics,
        prop_queried_true_metrics,
        prop_queried_false_metrics,
    )
...
parser.add_argument(
    "--domain", type=str, required=True, choices=["alchemy", "textworld"]
)
parser.add_argument(
    "--pred_files", type=str, help="comma-separated `.jsonl` file of model outputs"
)
parser.add_argument(
    "--use_remap_domain",
    action="store_true",
    default=False,
    help="evaluate on remap domain",
)
parser.add_argument(
    "--remap_fn",
    type=str,
    default=None,
    help="file path to remap probe outputs (required if `--use_remap_domain`)",
)
parser.add_argument(
    "--single_side_probe",
    action="store_true",
    default=False,
    help="`pred_files` include 1-side probe outputs",
)
...
if args.domain == "alchemy":
    ...
    for fn in eval_fns:
        ...
        with open(fn) as f:
            for line in f:
                ...
                if bn > 1:
                    ...
                    examples[ex_num]["n_beakers_correct"] += int(
                        line["gen_state"] == line["gold_state"]
                    )
                    ...
                    examples.append(
                        {
                            "prior": f'{init_ctxt}. {prior.split(". ")[-1]}',
                            "pred": line["gen_state"],
                            "gold": line["gold_state"],
                            "n_beakers_correct": int(
                                line["gen_state"] == line["gold_state"]
                            ),
                        }
                    )
        ...
        for ex in examples:
            ...
            ex_num_bkrmatch = 0
            for bkr in range(NUM_BEAKERS):
                ex_num_bkrmatch += gold_bkrs[bkr] == pred_bkrs[bkr]
...
if args.domain == "textworld":
    ...
    if args.single_side_probe:
        ...
        possible_pairs = load_possible_pairs(
            pair_out_file="../state-probes-TW/tw_games/training_traces_tw-simple/entity_pairs.json"
        )
    ...
    for eval_fn in eval_fns:
        ...
        if arch == "bart":
            tokenizer = BartTokenizerFast.from_pretrained(
                "facebook/bart-base", local_files_only=local_files_only
            )
        elif arch == "t5":
            tokenizer = T5TokenizerFast.from_pretrained(
                "t5-base", local_files_only=local_files_only
            )
        else:
        ...
        # get gts
        dataset = TWDataset(
            f"../state-probes-TW/tw_games/training_traces_tw-{data_type}",
            tokenizer,
            "dev",
            max_seq_len=512,
            max_data_size=500,
        ...
        for entry in dataset:
            if not control_input:
                ctxt = tokenizer.decode(
                    tokenizer.encode(entry["contexts"]), skip_special_tokens=True
                ).strip()
            else:
                ctxt = entry["contexts"].strip()
            ctxt_to_gt_state[ctxt] = {
                subkey: {
                    tf: parse_facts_to_nl(
                        entry["final_states"][subkey][tf], inform7_game
                    )
                    for tf in ["true", "false"]
                }
                for subkey in entry["final_states"]
                if "belief_facts" in subkey
            }
            ctxt_to_gt_state[ctxt]["full_facts"] = parse_facts_to_nl(
                entry["final_states"]["full_facts"], inform7_game
            )

        # get preds
        # by context
        results = {}
        queried_ent_em = [0.0, 0]
        cached_templates = {}
        ...
        for line in tqdm(lines):
            ...
            line["prev_context"] = line["prev_context"].strip()
            if (
                not args.use_remap_domain
                or line["entity"] in ctxt_to_remap_entities[line["prev_context"]]
            ):
                if line["prev_context"] not in results:
                    ...
                    results[line["prev_context"]] = {
                        ...
                        "queried_entities": [],
                        "gt_state": {
                            "true": ctxt_to_gt_state[line["prev_context"]][
                                "belief_facts"
                            ]["true"],
                            "false": ctxt_to_gt_state[line["prev_context"]][
                                "belief_facts"
                            ]["false"],
                        },
                        "full_gt_state": ctxt_to_gt_state[line["prev_context"]][
                            "full_facts"
                        ],
                    }
                ...
                if args.single_side_probe:
                    ...
                    queried_gt_state = {"true": [], "false": []}
                    for tf in ["true", "false"]:
                        gt_facts, cached_templates = parse_nl_to_facts(
                            line["gt_state"][tf],
                            game_state,
                            line["game_id"],
                            cached_templates=cached_templates,
                        ...
                        gen_facts, cached_templates = parse_nl_to_facts(
                            line["gen_state"][tf],
                            game_state,
                            line["game_id"],
                            cached_templates=cached_templates,
                    ...
                    for ent_list in all_entities_list:
                        ...
                        if (
                            not entset.has_nonNone
                            or entset not in possible_pairs[line["game_id"]]
                        ):
                            continue
                        for tf in ["true", "false"]:
                            queried_gt_state[tf] += parse_facts_to_nl(
                                get_relevant_facts_about(
                                    entset,
                                    line["gt_state"][tf],
                                    None,
                                    None,
                                    exact_arg_count=True,
                                    exact_arg_order=False,
                                ),
                                inform7_game,
                            )
                            queried_gen_state[tf] += parse_facts_to_nl(
                                get_relevant_facts_about(
                                    entset,
                                    line["gen_state"][tf],
                                    None,
                ...
                for tf in ["true", "false"]:
                    results[line["prev_context"]]["queried_gt_state"][tf] += line[
                        "gt_state"
                    ][tf]
                    results[line["prev_context"]]["queried_gen_state"][tf] += line[
                        "gen_state"
                    ][tf]
                results[line["prev_context"]]["queried_entities"].append(line["entity"])
                results[line["prev_context"]]["game_id"] = line["game_id"]
                ent_em = (line["gt_state"]["true"] == line["gen_state"]["true"]) and (
                    line["gt_state"]["false"] == line["gen_state"]["false"]
                )
        ...
        all_queried_true_metrics = torch.tensor([0.0, 0.0, 0.0])
        all_queried_false_metrics = torch.tensor([0.0, 0.0, 0.0])
        all_full_true_metrics = torch.tensor([0.0, 0.0, 0.0])
        all_full_false_metrics = torch.tensor([0.0, 0.0, 0.0])
        queried_em = 0
        full_em = 0
        relational_q_em = 0
        property_q_em = 0
        for context in results:
            if split_rel_prop:
                (
                    ...
                    results[context]["full_gt_state"],
                    results[context],
                    results[context]["game_id"],
                    game_state=game_state,
                    inform7_game=inform7_game,
                    cached_templates=cached_templates,
                )
                relational_q_em += (rel_queried_true_metrics[-1] == 1) and (
                    rel_queried_false_metrics[-1] == 1
                )
                property_q_em += (prop_queried_true_metrics[-1] == 1) and (
                    prop_queried_false_metrics[-1] == 1
                )
            # get overall ems
            queried_true_metrics, queried_false_metrics = get_prf1(
                results[context]["full_gt_state"],
                results[context]["queried_gen_state"]["true"],
                results[context]["queried_gen_state"]["false"],
                results[context]["queried_gt_state"]["true"],
                results[context]["queried_gt_state"]["false"],
            )
            queried_em += (queried_true_metrics[-1] == 1) and (
                queried_false_metrics[-1] == 1
            )
            ...
            if queried_true_metrics[-1] != 1:
                true_diff = set(
                    ...
                    set(results[context]["queried_gt_state"]["true"])
                )
            if queried_false_metrics[-1] != 1:
                false_diff = set(
                    results[context]["queried_gen_state"]["false"]
                ).symmetric_difference(
                    set(results[context]["queried_gt_state"]["false"])
                )
                ...
                for tf in ["true", "false"]:
                    for fact in results[context][state_key][tf]:
                        if "The matching key" in fact:
                            continue
                        if fact not in all_fact_counts:
                            all_fact_counts[fact] = 0
            ...
            full_true_metrics, full_false_metrics = get_prf1(
                ...
                results[context]["queried_gen_state"]["true"],
                results[context]["queried_gen_state"]["false"],
                results[context]["gt_state"]["true"],
                results[context]["gt_state"]["false"],
            )
