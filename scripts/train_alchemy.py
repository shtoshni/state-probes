...
COLORS_TO_INDEX = {color: index for (index, color) in enumerate(sorted(COLORS))}
...
parser.add_argument(
    "--encode_init_state", type=str, default=False, choices=[False, "raw", "NL"]
)
...
if args.arch == "bart":
    ...
    tokenizer = BartTokenizerFast.from_pretrained(
        model_fp, local_files_only=args.local_files_only
    )
elif args.arch == "t5":
...
dataset, lang_v, state_v = loadData(
    split="train", kind="alchemy", synthetic=args.synthetic
)
dev_dataset, lang_v_dev, state_v_dev = loadData(
    split="dev", kind="alchemy", synthetic=args.synthetic
)
best_val_loss = 10**10
best_val_consistency = 0.0
best_epoch = -1
all_train_states = [
    " ".join(state) for _, _, state in [x for d in dataset for x in d.all_pairs()]
]
all_dev_states = [
...
for i in range(args.epochs):
    ...
    for j, (inputs, lang_tgts, state_tgts, raw_state_targets, init_states) in enumerate(
        convert_to_transformer_batches(
            ...
            args.batchsize,
            random=random,
            include_init_state=args.encode_init_state,
            domain="alchemy",
            device=args.device,
        )
    ):
        ...
        lang_loss, dec_output, encoder_hidden = (
            return_dict.loss,
            return_dict.logits,
            return_dict.encoder_last_hidden_state,
        )
    ...
    print(
        f"epoch {i}, average lang loss {sum(lang_train_losses).item()/len(lang_train_losses)}"
    )
    model.eval()
    with torch.no_grad():
        tot_val_loss = 0
            ...
            if args.synthetic:
                try:
                    n_val_consistent += check_val_consistency(
                        ...
                        included_init_state=args.encode_init_state,
                    )
                except:
                    n_val_consistent += check_val_consistency(
                        model,
        ...
        print(
            f"epoch {i}, avg val loss: {avg_val_loss}, fraction consistent: {avg_val_consistency}"
        )

        if (args.synthetic and avg_val_consistency >= best_val_consistency) or (
            not args.synthetic and avg_val_loss <= best_val_loss
        ):
            ...
            print(
                f"model val {'consistency' if args.synthetic else 'loss'} went {'down' if args.synthetic else 'up'}"
            )
