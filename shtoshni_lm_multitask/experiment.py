...
class Experiment(object):
	...
	def train(self) -> None:
		...
		while True:
				...
				def handle_example():
					if self.args.rap_prob:
						if random.random() < 0.01:
							...
							logger.info(f"Auxilliary Decoder sequence: {self.tokenizer.decode(output_seq)}\n")

							output_seq = torch.clone(lang_tgts['input_ids'][0])
							output_seq.masked_fill_(output_seq == -100, self.tokenizer.pad_token_id)
							logger.info(f"Main Decoder sequence: {self.tokenizer.decode(output_seq)}\n")
					else:
						if random.random() < 0.01:
							logger.info(f"\nEncoder sequence: {self.tokenizer.decode(inputs['input_ids'][0])}")
							output_seq = torch.clone(lang_tgts['input_ids'][0])
					...
					loss = (return_dict_lang.loss + self.args.rap_prob * return_dict_state.loss)
					...
					return loss_val, return_dict_lang.loss.item(), return_dict_state.loss.item()

				mult_loss, lang_loss, state_loss = handle_example()
				if self.args.use_wandb:
					wandb.log({"train/loss": mult_loss, 'batch': self.train_info['global_steps']})
					wandb.log({"train/lang_loss": lang_loss, 'batch': self.train_info['global_steps']})
					wandb.log({"train/state_loss": state_loss, 'batch': self.train_info['global_steps']})
			...
			logger.info(f"epoch {self.train_info['num_epochs']}, avg loss {(sum(lang_train_losses) / len(lang_train_losses)):.3f}")
