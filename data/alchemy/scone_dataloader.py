...
def convert_to_transformer_batches(
    ...
    batchsize,
    random=None,
    include_init_state=False,
    no_context=False,
    append_last_state_to_context=False,
    domain="alchemy",
    state_targets_type=None,
    device="cuda",
    control_input=False,
):
    """
    ...
    for batch in batches:
        ...
        if state_targets_type_split[0] == "state":
            ...
            state_targets = {
                int(beaker.split(":")[0]) - 1: [] for beaker in states[0].split(" ")
            }
            for state in states:
                state_descr = decide_translate(
                    state,
                    state_targets_type,
                    domain,
                    isinstance(tokenizer, BartTokenizerFast),
        ...
        if control_input:
            ...
            inps = [
                ", ".join(
                    [f"the {int_to_word[num]} beaker is empty" for num in range(7)]
                )
                + "."
                for _ in inps
            ]
