    ...
    if domain == "alchemy":
        ...
        for bstate in all_beaker_states:
            if (
                translate_states_to_nl(
                    bstate,
                    domain,
                    not tokenizer or isinstance(tokenizer, BartTokenizerFast),
                )
                not in nl_all_beaker_states
            ):
                nl_all_beaker_states.add(
                    translate_states_to_nl(
                        bstate,
                        domain,
                        not tokenizer or isinstance(tokenizer, BartTokenizerFast),
        ...
        if encoding == "NL":
            ...
            if not tokenizer:
                ...
                tokenized_beaker_states = tokenizer(
                    nl_filtered_all_beaker_states,
                    return_tensors="pt",
                    padding=True,
                    truncation=True,
...
def gen_all_beaker_pos(domain, args, encoding="NL", tokenizer=None, device="cuda"):
    ...
    if domain == "alchemy" and encoding == "NL":
        # can have any beaker in any of 7 positions
        all_beaker_states = [
            f'{"" if i == 0 else " "}the {int_to_word[i]} beaker'
            for i in range(len(int_to_word))
        ]
        beaker_state_to_idx = {
            all_beaker_states[i]: i for i in range(len(all_beaker_states))
        }
        if not tokenizer:
            return all_beaker_states, beaker_state_to_idx
        else:
            tokenized_beaker_states = tokenizer(
                all_beaker_states, return_tensors="pt", padding=True, truncation=True
            ).to(device)
...
def get_matching_state_labels(
    all_states, beaker_state_to_idx, target_states, tokenizer, device="cuda"
):
    """
    ...
    for i in range(len(target_states["input_ids"])):
        target = tokenizer.decode(
            target_states["input_ids"][i], skip_special_tokens=True
        )
        labels.append(beaker_state_to_idx[target])

    labels = torch.tensor(labels).to(device)
    assert (
        all_states["input_ids"][labels][all_states["attention_mask"][labels].bool()]
        == target_states["input_ids"][target_states["attention_mask"].bool()]
    ).all()
    return labels
...
def encodeState(domain, state, device="cuda"):
    ...
    for contents in state:
        ...
        for lvl in range(height):
            x[0, ix] = 1
            x[1, colors.index(d_colors[contents[lvl]])] += 1
    ...
    if domain == "alchemy":
        ...
        for beaker_state in all_beakers:
            ...
            if "_" in beaker_state:
                ...
                nl_states.append(
                    f"the {beaker_number} beaker has {amount_in_beaker} {color_in_beaker}"
                )
            else:
                colors_to_amount = {}
        ...
        if len(nl_states) == 1:
            ...
            nl_states = (
                f"{' ' if beaker_number != 'first' and add_space else ''}{nl_states[0]}"
            )
        else:
...
def translate_nl_to_states(nl_inputs, domain):
    if domain == "alchemy":
        # non-well-formed
        if not check_well_formedness(nl_inputs):
            return ""
        all_beakers = nl_inputs.split(", ")
        raw_states = []
        for beaker_state in all_beakers:
            if "is" in beaker_state:
                beaker_state = beaker_state.split(" is ")
            elif "has" in beaker_state:
                beaker_state = beaker_state.split(" has ")
            else:
                assert False
            beaker_number = (
                word_to_int[beaker_state[0].split("the ")[1].split(" beaker")[0]] + 1
            )
            ...
            if "empty" in beaker_state:
                ...
                raw_states.append(
                    f"{beaker_number}:{''.join([color for _ in range(amount)])}"
                )
            else:
                string = ""
...
def decide_translate(probe_target, state_targets_type, domain, add_space=True):
    ...
    if len(state_targets_type) > 1 and state_targets_type[-1] == "NL":
        return translate_states_to_nl(probe_target, domain, add_space=add_space)
    else:
        return probe_target
