...
from data.textworld.utils import (
    apply_mask_and_truncate,
    control_pairs_simple,
    control_pairs_with_rooms_simple,
    EntitySet,
    ENTITIES_SIMPLE,
    ROOMS_SIMPLE,
    gen_possible_pairs,
    ...
    pad_stack,
    remap_entset,
    control_tgt_to_mention_simple,
    control_tgt_to_mention_with_rooms_simple,
)
...
class TWDataset(Dataset):
    def __init__(
        ...
        max_seq_len=512,
        max_data_size=10000,
        interleave_state_in_ctxt=False,
        pred_action_and_response_joint=False,
        inform7_game=None,
        randseed=None,
        logger=None,
        *args,
    ...
    def __getitem__(self, i):
        item = {k: self.data[k][i] for k in self.data}
        item["data_idx"] = i
        return item
    ...
    def load_data(self):
        ...
        files = sorted(
            glob.glob(
                os.path.join(
                    os.path.join(self.data_dir, self.data_split), "*_states.txt"
                )
            )
        )
        if self.randseed:
        ...
        for fp in tqdm(files):
            ...
            with open(langs_file) as f:
                ...
                if (
                    line.startswith(">")
                    and approx_num_toks + len(self.tokenizer.tokenize(line))
                    <= self.max_seq_len
                ):
            ...
            for c in range(2, n_cutoff_actions):
                ...
                increment_corresponding_state = all_actions[c - 1].startswith(
                    ">"
                )  # last action in context
        ...
        if self.logger:
            self.logger.info(f"Using files order: {init_actions_data['filenames']}")
...
class TWEntitySetDataset(TWDataset):
    ...
    def __init__(
        ...
        self.control_tgt_to_mention_with_rooms = (
            control_tgt_to_mention_with_rooms_simple
        )
        ...
        super().__init__(
            ...
            interleave_state_in_ctxt=interleave_state_in_ctxt,
            pred_action_and_response_joint=pred_action_and_response_joint,
    ...
    def find_mention_in_ctxt(self, entity, ctxt):
        ...
        for cand in candidates:
            ...
            if len(mention_location) > 0:
                if all_mention_locations is None:
                    all_mention_locations = []
    ...
    def load_data(self):
        ...
        if self.precomputed_negs is None:
            if self.ent_set_size == 2:
                self.possible_pairs = load_possible_pairs(
                    data_dir=self.gamefile, game_ids=self.get_gameids()
                )
            self.precomputed_negs = load_negative_tgts(
                data_dir=self.gamefile,
                tokenizer=self.tokenizer,
                game_ids=self.get_gameids(),
                ent_set_size=self.ent_set_size,
            )
        ...
        for i in tqdm(range(len(self.data["contexts"]))):
            ...
            init_state, tgt_state = (
                self.data["init_states"][i][self.state_key],
                self.data[self.tgt_state_key][i][self.state_key],
            )
            init_state = {
                tf: " [SEP] ".join(parse_facts_to_nl(init_state[tf], self.inform7_game))
            ...
            for e in self.all_entities:
                ...
                if self.control:
                    ce = (
                        self.control_tgt_to_mention_with_rooms[e]
                        if e in self.control_tgt_to_mention_with_rooms
                        else e
                    )
            ...
            all_entities_list = list(
                itertools.combinations([None, *entities], self.ent_set_size)
            )
            # create all entity pairs
            for ent_list in all_entities_list:
                entset = EntitySet(ent_list)
                if not entset.has_nonNone or (
                    self.possible_pairs is not None
                    and entset not in self.possible_pairs[game_id]
                ):
                    continue
                ...
                ent_facts = {}
                for tf in tgt_state:
                    relevant_facts = get_relevant_facts_about(
                        entset,
                        tgt_state[tf],
                        None,
                        None,
                        exact_arg_count=(self.ent_set_size > 1),
                ...
                if self.control == "control":
                    entset = EntitySet(
                        remap_entset(entset, self.control_tgt_to_mention)
                    )
                if self.control == "control_rooms":
                    entset = EntitySet(
                        remap_entset(entset, self.control_tgt_to_mention_with_rooms)
                    )
                ...
                ent_facts = {
                    tf: " [SEP] ".join(
                        parse_facts_to_nl(ent_facts[tf], self.inform7_game)
                    )
                    for tf in ent_facts
                }
                ...
                (
                    ...
                    all_states_inputs,
                    all_states_vectors,
                ) = self.get_matching_state_label(
                    entset, ent_facts, self.precomputed_negs
                )
    ...
    def get_matching_state_label(self, entset, target_state, precomputed_negs):
        ...
        (pos/neg/unk)
        """
        all_input_tokens = precomputed_negs["all_entity_inputs"][
            EntitySet.serialize(entset)
        ].to("cpu")
        all_vectors = precomputed_negs["all_entity_vectors"][
            EntitySet.serialize(entset)
        ]
                ...
                for fact in target_state[tf].split(" [SEP] "):
                    if "carries" in fact:
                        if fact not in fact_to_idx:
                            fact = (
                                fact.replace("The player carries ", "")
                                + " is in the inventory"
                            )
...
class TWEntitySetDataLoader(DataLoader):
    ...
    def tokenize_truncate(
        ...
        return {
            k: apply_mask_and_truncate(tokenized_inputs[k], mask, max_len, self.device)
    ...
    def collate_fn(self, batch):
        ...
        if self.control_input:
            ...
            for entity in batch["entities"]:
                ...
                mention = [
                    self.dataset.find_mention_in_ctxt(str(e), control_context[-1])
                    for e in entity
                ]
        ...
        items_to_keep = (
            context_tokens["attention_mask"].sum(1) <= self.tokenizer.model_max_length
        )
        if not items_to_keep.any():
            return None, None, None, None, batch["game_ids"], None
        context_tokens = {
            k: apply_mask_and_truncate(
                context_tokens[k],
                items_to_keep,
                self.tokenizer.model_max_length,
                self.device,
            )
            for k in context_tokens
        }
        # get lang tgts
        tgt_tokens = self.tokenize_truncate(
            batch["tgts"], items_to_keep, self.tokenizer.model_max_length
        )
        # get state tgts
        state_tokens = {}
        for state_type in ["init_states", "tgt_states"]:
            ...
            for tf in ["true", "false"]:
                tokenized_state = self.tokenize_truncate(
                    [state[tf] for state in batch[state_type]],
                    items_to_keep,
                    self.tokenizer.model_max_length,
                )
                state_token_key = f"{k}_{tf}" if tf == "false" else k
                for k in tokenized_state:
                    state_tokens[state_type][state_token_key] = tokenized_state[k]
        # mention sets/entity sets/gameids
        entity_sets = {
            "mentions": [
                ent for idx, ent in enumerate(batch["mentions"]) if items_to_keep[idx]
            ],
            "entities": [
                ent for idx, ent in enumerate(batch["entities"]) if items_to_keep[idx]
            ],
        }
        game_ids = [
            gid for idx, gid in enumerate(batch["game_ids"]) if items_to_keep[idx]
        ]

        # labels
        # (bs, # facts, seqlen[, embeddim])
        (
            state_tokens["tgt_states"]["all_states_input_ids"],
            state_tokens["tgt_states"]["all_states_attn_mask"],
        ) = pad_stack(
            batch["all_states_tokenized"],
            pad_idx=self.tokenizer.pad_token_id,
            device=self.device,
        )
        state_tokens["tgt_states"]["all_states_encoding"], encoding_mask = pad_stack(
            batch["all_states_encoded"], pad_idx=0, device=self.device
        )
        assert (
            encoding_mask == state_tokens["tgt_states"]["all_states_attn_mask"]
        ).all()
        max_nfacts = state_tokens["tgt_states"]["all_states_input_ids"].size(1)
        # (bs, # facts)
        labels = [
            lentry + [-1 for _ in range(max_nfacts - len(lentry))]
            for lentry in batch["labels"]
        ]
        labels = torch.tensor(labels).to(self.device)
        assert (
            (labels != -1)
            == state_tokens["tgt_states"]["all_states_attn_mask"][:, :, 0]
        ).all()
        ...
        return (
            context_tokens,
            tgt_tokens,
            state_tokens["init_states"],
            state_tokens["tgt_states"],
            game_ids,
            entity_sets,
        )
...
class TWFullDataLoader(DataLoader):
    def __init__(
        ...
        if len(state_keys_to_get) == 0:
            ...
            self.state_keys = [
                key[1].replace("_single", "").replace("_pair", "")
                for key in state_keys_to_get
            ]
    ...
    def collate_fn(self, batch):
        ...
        context_tokens = {
            key: apply_mask_and_truncate(
                context_tokens[key],
                items_to_keep,
                self.tokenizer.model_max_length,
                self.device,
            )
            for key in context_tokens
        }
        tgts = [item["tgts"] for item in batch]
        tgt_tokens = self.tokenizer(
            tgts, return_tensors="pt", padding=True, truncation=True
        )
        ...
        tgt_tokens = {
            ...
            if type(tgt_tokens[key]) == torch.Tensor
            else tgt_tokens[key]
        ...
        for sk, state_key in enumerate(self.state_keys):
            if "belief_facts" in state_key:
                init_states[state_key] = {
                    tf: [] for tf in batch[0]["init_state"][state_key]
                }
                final_state[state_key] = {
                    tf: [] for tf in batch[0][self.tgt_state_keys[sk]][state_key]
                }
            ...
            for j in range(len(batch)):
                ...
                init_state, tgt_state = (
                    batch[j][init_state_key][state_key],
                    batch[j][self.tgt_state_keys[sk]][state_key],
                )
                if type(init_state) != dict:
                    init_state, tgt_state = {"true": init_state}, {"true": tgt_state}
                env = textworld.start(
                    os.path.join(self.gamefile, f"{game_ids[j-i]}.ulx")
                )
                game_state = env.reset()
                inform7_game = env._inform7
                for tf in init_state:
                    init_facts_gold = " [SEP] ".join(
                        parse_facts_to_nl(init_state[tf], inform7_game)
                    )
                    if j >= max_gt_grounded_states:
                        init_facts = ""
                    else:
                        init_facts = init_facts_gold
                    init_states[state_key][tf].append(init_facts)
                for tf in tgt_state:
                    tgt_facts_gold = " [SEP] ".join(
                        parse_facts_to_nl(tgt_state[tf], inform7_game)
                    )
                    if j >= max_gt_grounded_states:
                        tgt_facts = ""
                    else:
        ...
        for state_key in init_states:
            ...
            for tf in init_states[state_key]:
                tokenized_init_tf = self.tokenizer(
                    init_states[state_key][tf],
                    return_tensors="pt",
                    padding=True,
                    truncation=True,
                ).to(self.device)
                tokenized_tgt_tf = self.tokenizer(
                    final_state[state_key][tf],
                    return_tensors="pt",
                    padding=True,
                    truncation=True,
                ).to(self.device)
                for k2 in tokenized_init_tf:
                    init_state_tokens[state_key][
                        f'{k2}{"_"+tf if tf != "true" else ""}'
                    ] = tokenized_init_tf[k2]
                    tgt_state_tokens[state_key][
                        f'{k2}{"_"+tf if tf != "true" else ""}'
                    ] = tokenized_tgt_tf[k2]
