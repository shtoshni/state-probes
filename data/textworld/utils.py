...
def pad_stack(inputs, pad_idx=1, device="cpu"):
    ...
    for i, inp in enumerate(inputs):
        ...
        new_mask = torch.cat(
            [
                inp["attention_mask"],
                torch.zeros(*mask_size).to(
                    inp["attention_mask"].device, inp["attention_mask"].dtype
                ),
            ],
            dim=1,
        )
        ...
        new_mask = torch.cat(
            [
                new_mask,
                torch.zeros(mask_size[0], max_seqlen, *mask_size[2:]).to(
                    inp["attention_mask"].device, inp["attention_mask"].dtype
                ),
            ],
            dim=0,
        ...
        new_inp = (
            torch.ones(max_nfacts, max_seqlen, *inp["input_ids"].size()[2:]).to(
                inp["input_ids"].device, inp["input_ids"].dtype
            )
            * pad_idx
        )
...
def get_relevant_facts_about(
    ...
    for fact in facts:
        ...
        if exact_arg_count and len(fact_argnames) != count_nonNone_entities:
            continue  # argument count doesn't match
        if "I" in fact_argnames:
            fact_argnames[fact_argnames.index("I")] = "inventory"
        if "P" in fact_argnames:
            fact_argnames[fact_argnames.index("P")] = "player"
        ...
        for e, entity in enumerate(entities):
            if entity is not None and (
                (exact_arg_order and entity != fact_argnames[e])
                or (not exact_arg_order and entity not in fact_argnames)
            ):
                add_fact = False
                continue
        if add_fact:
...
def load_possible_pairs(pair_out_file=None, data_dir=None, game_ids=None):
    if pair_out_file and os.path.exists(pair_out_file):
        ...
        for gameid in possible_pairs_serialized:
            possible_pairs[gameid] = [
                EntitySet.deserialize(pair_str)
                for pair_str in possible_pairs_serialized[gameid]
            ]
...
def load_negative_tgts(
    negative_tgts_fn=None,
    data_dir=None,
    ent_set_size=None,
    tokenizer=None,
    game_ids=None,
    ...
    if negative_tgts_fn and os.path.exists(negative_tgts_fn):
        ...
        return gen_all_facts(
            data_dir, state_encoder, None, tokenizer, game_ids, ent_set_size
        )
    else:
        return None
...
def remap_entset(entset, control_mapping):
    ...
    for e, entity in enumerate(entset):
        ...
        if entity is not None:
            entity_name = (
                control_mapping[entity_name]
                if entity_name in control_mapping
                else entity_name
            )
...
def gen_possible_pairs(data_dir, game_ids):
    ...
    for game_id in tqdm(game_ids):
        ...
        for t in game_types.types:
            if t not in type_to_gid_to_ents[game_id]:
                type_to_gid_to_ents[game_id][t] = []
            type_to_gid_to_ents[game_id][t] += [
                game_ents[e].name
                for e in game_ents
                if game_types.is_descendant_of(game_ents[e].type, t)
            ]
    ...
    for game_id in tqdm(game_ids):
        ...
        for signature in predicates:
            if len(predicates[signature][1]) == 0:
                continue
            if signature.types in type_pairs:
                continue
            type_pairs[game_id].add(signature.types)
            obj_pairs = (
                set(
                    itertools.product(
                        type_to_gid_to_ents[game_id][signature.types[0]],
                        type_to_gid_to_ents[game_id][signature.types[1]],
                    )
                )
                if len(signature.types) == 2
                else set(
                    itertools.product(
                        type_to_gid_to_ents[game_id][signature.types[0]], [None]
                    )
                )
            )
...
def gen_all_facts(
    gamefile, state_encoder, probe_outs, tokenizer, game_ids, ent_set_size, device
):
    game_id_to_entities = {}
    game_id_to_objs = {}
    game_ids_to_kb = {}
...
def gen_all_facts_single(
    ...
    for game_id in game_ids:
        ...
        for signature in predicates:
            ...
            if len(predicates[signature][0].parameters) > 2:
                continue
                ...
                for b in range(len(signature.types)):
                    blanks_possibles.append(
                        [
                            ...
                            for b2, typ in enumerate(
                                signature.types
                            )  # if game_ids_to_kb[game_id].types.is_constant(typ)
                        ]
                    )
                    for arg_combines in itertools.product(*blanks_possibles[b]):
                        ...
                        for a, arg in enumerate(arg_combines):
                            arg_template_name = (
                                predicates[signature][0].parameters[a].name
                            )
                            assert "{" + arg_template_name + "}" in pred_str
                            pred_str = pred_str.replace(
                                "{" + arg_template_name + "}", arg
                            )
                        fact_to_template[pred_str] = signature
                        all_facts.append(pred_str)
            else:
                pred_str = predicates[signature][1]
                for t, typ in enumerate(
                    signature.types
                ):  # necessary to delete the type-specific templates
        ...
        for typ in game_id_to_entities[game_id]:
            if typ == "I":
                entity_name_to_types["inventory"] = {"I"}
                continue
            if typ == "P":
                entity_name_to_types["player"] = {"P"}
                continue
            for name in game_id_to_entities[game_id][typ]:
                if name is not None:
                    if name not in entity_name_to_types:
                        entity_name_to_types[name] = set()
                    if "-=" in name:
                        name = name[3:-3].lower()
                    entity_name_to_types[name].add(var_names[typ])
    all_facts = list(set(all_facts))

    if probe_outs is None:
        probe_outs = {}
    ...
    for entity in tqdm(entity_name_to_types):
        ...
        probe_outs["idx_to_state"][entset_serialize] = list(
            set(probe_outs["idx_to_state"][entset_serialize])
        )
        probe_outs["state_to_idx"][entset_serialize] = {
            fact: i
            for i, fact in enumerate(probe_outs["idx_to_state"][entset_serialize])
        }
        # input_ids: ([bs *] # facts, seqlen), attention_mask: ([bs *] # facts, seqlen)
        probe_outs["all_entity_inputs"][entset_serialize] = tokenizer(
            probe_outs["idx_to_state"][entset_serialize],
            return_tensors="pt",
            padding=True,
            truncation=True,
        ...
        for split in range(
            0,
            probe_outs["all_entity_inputs"][entset_serialize]["input_ids"].size(0),
            SPLIT_SIZE,
        ):
            inp_ids = probe_outs["all_entity_inputs"][entset_serialize]["input_ids"][
                split : split + SPLIT_SIZE
            ]
            attn_mask = probe_outs["all_entity_inputs"][entset_serialize][
                "attention_mask"
            ][split : split + SPLIT_SIZE]
            encoded_inputs.append(
                state_encoder(
                    input_ids=inp_ids, attention_mask=attn_mask, return_dict=True
        ...
        probe_outs["all_entity_vectors"][entset_serialize] = {
            ...
            "attention_mask": probe_outs["all_entity_inputs"][entset_serialize][
                "attention_mask"
            ].to("cpu"),
        }
...
def gen_all_facts_pairs(
    ...
    if len(entity_name_to_types) > 50:
        ent_pairs = tqdm(ent_pairs)
    for ent_pair in ent_pairs:
        ...
        if entset_serialize not in batch_all_facts:
            batch_all_facts[entset_serialize] = []
        ...
        for fact in all_facts:
            ...
            if entset.has_none:
                ...
                if "entity0" in fact and "entity1" in fact:
                    continue
                # fill whichever is present
                fact_filled = fact.replace("entity0", entset.nonNone_ent).replace(
                    "entity1", entset.nonNone_ent
                )
            else:
                # for facts with 1 arg, ent_pair cannot have 2 args
                if "entity0" not in fact or "entity1" not in fact:
                    continue
                # both present
                fact_filled = fact.replace("entity0", entset[0]).replace(
                    "entity1", entset[1]
                )
                fact_filled_reverse = fact.replace("entity0", entset[1]).replace(
                    "entity1", entset[0]
                )
                batch_all_facts[entset_serialize].append(fact_filled_reverse)
            batch_all_facts[entset_serialize].append(fact_filled)
        fact_to_idx[entset_serialize] = {
            fact: idx for idx, fact in enumerate(batch_all_facts[entset_serialize])
        }
        # input_ids: ([bs *] # facts, seqlen), attention_mask: ([bs *] # facts, seqlen)
        token_all_facts[entset_serialize] = tokenizer(
            batch_all_facts[entset_serialize],
            return_tensors="pt",
            padding=True,
            truncation=True,
...
control_pairs_simple = [
    ("player", "inventory"),
    ("inventory", "player"),
    ("wooden door", "screen door"),
    ("screen door", "refrigerator"),
    ("refrigerator", "counter"),
    ("counter", "stove"),
    ("stove", "kitchen island"),
    ("kitchen island", "apple"),
    ("apple", "note"),
    ("note", "tomato plant"),
    ("tomato plant", "wooden door"),
    ("bell pepper", "milk"),
    ("milk", "shovel"),
    ("shovel", "half of a bag of chips"),
    ("half of a bag of chips", "bell pepper"),
    ("toilet", "bath"),
    ("bath", "sink"),
    ("sink", "soap bar"),
    ("soap bar", "toothbrush"),
    ...
    ("tv", "remote"),
    ("remote", "lettuce"),
    ("chest drawer", "antique trunk"),
    ("antique trunk", "king-size bed"),
    ("king-size bed", "old key"),
    ("old key", "chest drawer"),
    ("set of chairs", "bbq"),
    ("bbq", "patio table"),
    ("patio table", "set of chairs"),
]
control_pairs_with_rooms_simple = control_pairs_simple + [
    ("garden", "bathroom"),
    ("bathroom", "kitchen"),
    ("kitchen", "bedroom"),
    ("bedroom", "backyard"),
...
control_tgt_to_mention_with_rooms_simple = {
    pair[0]: pair[1] for pair in control_pairs_with_rooms_simple
}
control_mention_to_tgt_simple = {pair[1]: pair[0] for pair in control_pairs_simple}
control_mention_to_tgt_with_rooms_simple = {
    pair[1]: pair[0] for pair in control_pairs_with_rooms_simple
}
