...
def translate_inv_items_to_str(inv_items):
    ...
    if len(inv_items) == 0:
        ...
        inv_str = f"You are carrying: {inv_items[0]}."
...
def translate_inv_str_to_items(inv_str):
    ...
    if inv_str == "You are carrying nothing.":
        return []
...
def parse_nl_to_facts(
    ...
    game_type_to_entities = {
        t: [
            game_ents[e].name
            for e in game_ents
            if game_types.is_descendant_of(
                game_ents[e].type, t
            )  # game_ents[e].type == t  #
        ]
    ...
    if gameid not in cached_templates:
        ...
        for signature in predicates:
            ...
            if len(nl_template) == 0:
                continue
            param_indices = [
                [m.start(), m.end()] for m in re.finditer(entity, nl_template)
            ]
            symbol_to_type = {
                param.name: param.type for param in predicates[signature][0].parameters
            }
            for p in range(len(param_indices) - 1, -1, -1):
                param_index_pair = param_indices[p]
                ent_type = symbol_to_type[
                    nl_template[param_index_pair[0] + 1 : param_index_pair[1] - 1]
                ]
            ...
            param_idx_to_nl_position = (
                []
            )  # param idx in predicate -> position (group #) in nl string
            ...
            param_idx_to_nl_position = [
                i[0]
                for i in sorted(enumerate(param_idx_to_nl_position), key=lambda x: x[1])
            ]
            if regex in cached_templates[gameid]:
                assert (
                    param_idx_to_nl_position
                    == cached_templates[gameid][regex]["param_idx_to_nl_position"]
                )
                assert (
                    predicates[signature][0].name
                    == cached_templates[gameid][regex]["predicate"].name
                )
                assert (
                    predicates[signature][0].parameters[0].name
                    == cached_templates[gameid][regex]["predicate"].parameters[0].name
                )
                if len(predicates[signature][0].parameters) > 1:
                    assert (
                        predicates[signature][0].parameters[1].name
                        == cached_templates[gameid][regex]["predicate"]
                        .parameters[1]
                        .name
                    )
    ...
    for nl_fact in nl_facts:
        ...
        for matched_template in cached_templates[gameid]:
            if re.fullmatch(matched_template, nl_fact):
                break
        ...
        for param in cached_templates[gameid][matched_template]["predicate"].parameters:
            if param.name != "I" and param.name != "P":
                entity_name = re.search(matched_template, nl_fact).group(
                    cached_templates[gameid][matched_template][
                        "param_idx_to_nl_position"
                    ][param_idx]
                    + 1
                )
        ...
        fact = Proposition.serialize(
            cached_templates[gameid][matched_template]["predicate"].instantiate(mapping)
        )
        facts.append(fact)
    ...
    if len(test_nl_facts) == len(facts):
        try:
            ...
            test_nl_facts = {
                fact
                if "The player carries" not in fact
                else f'T{fact.replace("The player carries t", "")} is in the inventory'
                for fact in test_nl_facts
            }
            assert test_nl_facts == set(nl_facts) - set(invalid_syntax_facts)
