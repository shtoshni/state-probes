...
def probing_exp(model_path: str, base_dir: str):
    ...
    cloze_seq_ids = tokenizer.batch_encode_plus(
        cloze_steps, padding=True, add_special_tokens=True, return_tensors="pt"
    )["input_ids"].to(device)
    ...
    output_file = path.join(
        path.dirname(path.dirname(model_path.rstrip("/"))), "cloze_mrr_fine.txt"
    )
    ...
    with open(output_file, "w") as f:
        for idx, (init_state, prev_actions, cur_state, next_action) in enumerate(
            ...
            lang_loss = loss_fct(
                lm_logits.view(-1, len(tokenizer)), cloze_seq_ids.view(-1)
            )
    ...
    output_file = path.join(
        path.dirname(path.dirname(model_path.rstrip("/"))), "cloze_mrr.txt"
    )
    with open(output_file, "w") as f:
        f.write(f"{mrr:.3f}")
